...
import { Menu, X, Sparkles, Code, Users, Zap, Home, LogOut, KeyRound, BarChart2, User2 } from 'lucide-react'
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden lg:flex items-center space-x-2">
            {navItems.map((item, index) => (
              <motion.div key={item.to} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
                <NavLink
                  to={item.to}
                  className={({ isActive }) => `flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium ${isActive ? 'text-gray-900 bg-gray-100' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'}`}
                >
            ...
            {!isAuthenticated ? (
              <div className="flex items-center gap-2">
                <Link to="/customer/login" className="px-3 py-2 text-sm font-medium text-gray-600 hover:text-gray-900">Sign in</Link>
                <Link to="/customer/signup" className="px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg hover:bg-gray-800">Sign up</Link>
              </div>
            ) : (
              ...
              <div className="flex items-center gap-2">
                <div className="hidden xl:block px-3 py-2 text-sm text-gray-600">{customer?.email || 'Customer'}</div>
                <NavLink
              ...
              <div className="flex flex-col space-y-4">
                {navItems.map((item, index) => (
                  <motion.div key={item.to} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }}>
                    <NavLink
                      to={item.to}
                      className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${isActive ? 'text-gray-900 bg-gray-100' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'}`}
                ...
                {!isAuthenticated ? (
                  <div className="flex flex-col gap-2 px-4">
                    <Link to="/customer/login" className="px-4 py-3 text-center rounded-lg bg-gray-900 text-white" onClick={()=>setIsMenuOpen(false)}>Sign in</Link>
                    <Link to="/customer/signup" className="px-4 py-3 text-center rounded-lg bg-gray-100 text-gray-900" onClick={()=>setIsMenuOpen(false)}>Sign up</Link>
                  </div>
                ) : (
